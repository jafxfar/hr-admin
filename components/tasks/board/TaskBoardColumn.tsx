...
import type { TaskBoardColumn as BoardColumn } from '@/types/taskBoard'
import type { TaskItem } from '@/types/tasks'
import { TaskBoardCard, cardDragId } from './TaskBoardCard'
...
export const TaskBoardColumn = ({
  ...
  const itemIds = column.tasks.map((task) => cardDragId(task.id))

  const handleAddClick = () => {
    if (isAddDisabled || !onAddTask) return
    onAddTask(column.status.code)
  }
    ...
    >
      <div className="flex shrink-0 items-center gap-2 px-4 pt-4 pb-3">
        ...
        <span
          ...
          aria-label={`Количество: ${column.tasks.length}`}
        >
          {column.tasks.length}
        </span>
        {onAddTask ? (
          <button
            ...
            aria-label={`Добавить задачу в «${column.status.title}»`}
      ...
      >
        <SortableContext items={itemIds} strategy={verticalListSortingStrategy}>
          {column.tasks.map((task) => (
            <TaskBoardCard key={task.id} task={task} onOpen={onOpenCard} />
          ))}
        </SortableContext>
