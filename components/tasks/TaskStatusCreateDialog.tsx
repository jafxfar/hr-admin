...
type TaskStatusCreateDialogProps = {
  isOpen: boolean
  isPending?: boolean
  errorMessage?: string | null
  editingStatus?: TaskStatusItem | null
  onClose: () => void
  onSubmitCreate: (data: CreateTaskStatusDTO) => void
  onSubmitUpdate?: (code: string, data: UpdateTaskStatusDTO) => void
}
...
export const TaskStatusCreateDialog = ({
  isOpen,
  isPending,
  errorMessage,
  editingStatus = null,
  ...
  const handleSubmit = () => {
    ...
    if (isEditMode && editingStatus) {
      ...
      onSubmitUpdate(editingStatus.code, {
        title: trimmedTitle,
        is_active: isActive,
      ...
      <DialogContent className="select-text border border-app-border-accent bg-app-surface-0 text-app-text sm:max-w-md">
        ...
        <div className="space-y-4 py-2">
          ...
          {isEditMode ? (
            <StatusToggle
              label="По умолчанию"
              description="Статус для новых задач без указания колонки"
              checked={isDefault}
